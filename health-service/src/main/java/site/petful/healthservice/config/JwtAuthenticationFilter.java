...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // 토큰이 없거나 유효하지 않은 경우 401 에러로 차단
        log.warn("JWT Filter - No valid token found, blocking request to: {}", request.getRequestURI());
        response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
        response.setContentType("application/json;charset=UTF-8");
        response.getWriter().write("{\"success\":false,\"errorCode\":\"UNAUTHORIZED\",\"message\":\"인증이 필요합니다.\",\"data\":null}");
        return;
    }
    ...
    private boolean validateToken(String token) {
        try {
            ...
            Jwts.parser()
                .verifyWith(Keys.hmacShaKeyFor(Decoders.BASE64.decode(accessSecret.trim())))
                .build()
